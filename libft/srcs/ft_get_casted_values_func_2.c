...
{
	pf->var.integer = (int)va_arg(va, int64_t);
}

void	ft_gtvar_signed_char(t_pf *pf, va_list va)
{
	pf->var.integer = (signed char)va_arg(va, int64_t);
}

void	ft_gtvar_short_int(t_pf *pf, va_list va)
{
	pf->var.integer = (short)va_arg(va, int64_t);
}

void	ft_gtvar_size_t(t_pf *pf, va_list va)
{
	pf->var.integer = (size_t)va_arg(va, int64_t);
}

void	ft_gtvar_long_int(t_pf *pf, va_list va)
{
	pf->var.integer = (long int)va_arg(va, int64_t);
}
